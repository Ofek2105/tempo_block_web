  ...
  const render = (ctx) => {
    ...
    bullets.forEach(bullet => {
      if (bullet.trail.length > 1) {
        ctx.strokeStyle = GAME_CONFIG.BULLET_TRAIL_COLOR;
        ctx.lineWidth = 2;
        ctx.lineCap = 'round';
        ctx.beginPath();
    ...
    // Draw bullets
    bullets.forEach(bullet => {
      ctx.fillStyle = GAME_CONFIG.BULLET_COLOR;
      ctx.shadowColor = GAME_CONFIG.BULLET_COLOR;
      ctx.shadowBlur = 10;
      ctx.beginPath();
    ...
    // Draw shield
    ctx.strokeStyle = GAME_CONFIG.SHIELD_COLOR;
    ctx.shadowColor = GAME_CONFIG.SHIELD_GLOW_COLOR;
    ctx.shadowBlur = 15;
    ctx.lineWidth = GAME_CONFIG.SHIELD_WIDTH;
    ctx.lineCap = 'round';
    ctx.beginPath();
    ctx.arc(shield.x, shield.y, shield.radius, 
           shieldAngleRef.current - shield.arcSize / 2, 
           shieldAngleRef.current + shield.arcSize / 2);
    ctx.stroke();
    ctx.shadowBlur = 0;
